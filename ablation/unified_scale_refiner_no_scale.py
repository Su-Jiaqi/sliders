...
class UnifiedScaleRefinerNoScaleDataset(Dataset):
    """
    Ablation B:
      input: [gen_s(3), pre(3)] => 6 channels
      NO scale_map input

    target:
      s=0 -> pre
      s=1 -> post
      mid -> pseudo_root/scaleX/stem.png
    """
    ...
    def __init__(
        ...
        pseudo_maps: Dict[str, Dict[str, Path]],
        image_size: int = 256,
    ):
        self.samples = stems
    ...
    def __getitem__(self, idx):
        ...
        if abs(s) < 1e-8:
            ...
            target = load_rgb(self.pseudo_maps[scale_name][stem], self.image_size)
            target_kind = 2

        x = torch.cat([gen, pre], dim=0)  # 6 channels only
        ...
        return {
            ...
            "pre": pre,
            "target": target,
            "scale": torch.tensor(float(s), dtype=torch.float32),  # still keep for loss weighting / logging
...
class UnifiedResidualRefinerUNet(nn.Module):
    def __init__(self, in_ch: int = 6, base_ch: int = 64, residual_scale: float = 0.50):
        super().__init__()
...
def build_scale_and_pseudo_maps(cfg: TrainConfig):
    ...
    pseudo_root = resolve_path(cfg.pseudo_root)
    ...
    for scale_dir in scale_dirs:
        ...
        common = set(pre_map) & set(gen_map)
        if abs(s - 1.0) < 1e-8:
            common &= set(post_map)
        elif abs(s) > 1e-8:
            pseudo_dir = pseudo_root / scale_name
            if not pseudo_dir.exists():
                raise ValueError(f"Missing pseudo dir for {scale_name}: {pseudo_dir}")
            pseudo_map = list_image_map(pseudo_dir)
            pseudo_maps[scale_name] = pseudo_map
            common &= set(pseudo_map)

        for stem in sorted(common):
            all_pairs.append((stem, scale_name))

    if not all_pairs:
        raise ValueError("No training pairs found across pre/post/scale/pseudo roots.")
    return pre_map, post_map, scale_maps, pseudo_maps, all_pairs


def build_datasets(cfg: TrainConfig):
    pre_map, post_map, scale_maps, pseudo_maps, all_pairs = build_scale_and_pseudo_maps(cfg)
    train_pairs, val_pairs = split_pairs(all_pairs, cfg.val_ratio, cfg.seed)

    train_ds = UnifiedScaleRefinerNoScaleDataset(
        stems=train_pairs,
        pre_map=pre_map,
        post_map=post_map,
        scale_maps=scale_maps,
        pseudo_maps=pseudo_maps,
        image_size=cfg.image_size,
    )
    val_ds = UnifiedScaleRefinerNoScaleDataset(
        stems=val_pairs,
        pre_map=pre_map,
        post_map=post_map,
        scale_maps=scale_maps,
...
def compute_losses(
    ...
    # keep the same loss design as original unified refiner:
    # endpoint samples use lambda_lpips_endpoint, middle scales use lambda_lpips_mid
...
def train(cfg: TrainConfig):
    ...
    meta = {
        ...
        "ablation": "w/o_scale_conditioning",
    }
...
def build_parser():
    parser = argparse.ArgumentParser("Unified refiner ablation B: without scale conditioning")
    ...
    p_train.add_argument("--pseudo_root", type=str, required=True)
