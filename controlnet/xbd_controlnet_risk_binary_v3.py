#!/usr/bin/env python3
"""
xbd_controlnet_risk_binary_v3.py

A stronger version of the original starter script for paired wildfire pre->post
translation on xBD-style data.

What is changed vs. the original starter:
1) Still trains a ControlNet initialized from SD1.5 UNet.
2) Optionally trains UNet LoRA attention processors at the same time.
3) Adds direct reconstruction supervision on predicted x0:
   - image-space L1 loss
   - optional LPIPS perceptual loss
4) Adds an explicit color/filter loss on decoded predicted x0 vs. post image:
   - low-frequency pooled RGB loss
   - per-channel mean/std color moment loss
   This helps pull generated color tone away from green pre-image bias and
   closer to the post-disaster target's global palette.
5) Keeps the original binary severity mechanism, but also supports different
   prompts for post and identity branches.
6) Saves both ControlNet weights and UNet LoRA weights for sampling.

Typical usage for a strong post-only sanity/overfit test:
...
Requirements:
    ...
    pip install lpips   # only needed if --lambda_lpips > 0
"""
...
class XBDRiskBinaryDataset(Dataset):
    """
    By default, for each pair (pre, post), this dataset creates:
      - severity=1 -> control=pre, target=post, prompt=post_prompt
      - severity=0 -> control=pre, target=pre,  prompt=identity_prompt

    For strict post-only training / overfit testing, use --no_identity_samples.
    """
...
def attach_unet_lora(unet: UNet2DConditionModel, rank: int = 8, alpha: int = 8) -> AttnProcsLayers:
    """
    Attach LoRA attention processors to every attention module in the UNet.
    Returns an AttnProcsLayers wrapper whose parameters are the trainable LoRA params.
    """
    lora_attn_procs = {}
    ...
    for name in unet.attn_processors.keys():
        ...
        if name.startswith("mid_block"):
            ...
            # Fallback for any custom/unexpected key.
            hidden_size = block_out_channels[-1]
...
def maybe_build_lpips(device: torch.device, net: str = "alex"):
    try:
        ...
        raise ImportError(
            "LPIPS requested but package 'lpips' is not installed. "
...
def predict_x0_from_eps(
    ...
    """
    For epsilon-prediction DDPM schedulers:
...
def compute_color_loss(
    ...
    """
    Global color/filter loss.

    We intentionally ignore fine texture here and compare only low-frequency
    color information plus per-channel color moments. This is meant to make
    the decoded prediction inherit the *post image's palette / tone* rather
    than staying overly green like the pre image.
    ...
    loss_moment = (
        mean_weight * F.l1_loss(pred_mean, target_mean, reduction="mean")
        + std_weight * F.l1_loss(pred_std, target_std, reduction="mean")
    )

    return loss_low + loss_moment
...
def train(args):
    ...
    while state.global_step < args.max_train_steps:
        ...
        if args.lambda_l1 > 0.0 or args.lambda_lpips > 0.0 or args.lambda_color > 0.0:
            pred_x0_latents = predict_x0_from_eps(
...
def build_parser():
    ...
    # Loss weights
    p_train.add_argument("--lambda_eps", type=float, default=1.0)
    p_train.add_argument("--lambda_l1", type=float, default=1.0)
    p_train.add_argument("--lambda_lpips", type=float, default=0.25)
    p_train.add_argument("--lambda_color", type=float, default=0.5, help="Global color/filter loss weight")
    p_train.add_argument("--color_loss_size", type=int, default=32, help="Low-frequency pooled size for color loss")
