...
def configure_target_modules(
    ...
    # 关键：原地修改全局 list，让 LoRANetwork.__init__ 内部真正读到它
...
def build_scheduler() -> DDIMScheduler:
    # 手动创建，避免离线环境去下载 scheduler_config.json
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Correct img2img inference for your LoRA slider")
    ...
    p.add_argument(
        "--prompt",
        default="aerial satellite image of a post-wildfire mountain region, scorched vegetation, burn scar, charred terrain, ash-covered ground, remote sensing image",
    )
    p.add_argument(
        "--negative_prompt",
        default="oversharpened, oversaturated, unrealistic texture, artifacts, distorted terrain, duplicated ridges, painting, cartoon, fake satellite image, blurry, low quality",
    )

    p.add_argument("--scales", default="0,0.2,0.4,0.6,0.8")
    ...
    p.add_argument("--output_dir", default="outputs/socal_fire_img2img_fixed")
...
def main() -> None:
    ...
    for img_path in tqdm(input_paths, desc="images"):
        ...
        for scale in scales:
            ...
            outputs.append(out)
            per_scale_path = out_dir / f"{safe_name(img_path.stem)}_scale{scale}.png"
