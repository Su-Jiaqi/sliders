...
#
# v3: replaces v2's severity-probe-guided extrapolation loss (a frozen-classifier
# score-matching MSE, found empirically NOT to fix the non-monotonic top-of-range
# trajectory -- see calibrated_severity_final_evaluation_results.md, "v2
# improvement" section) with a direct pixel-space monotonicity ordering loss.
# Rationale: v2's gradient had to flow through a frozen classifier + linear probe,
# which is a weak, indirect signal relative to the main diffusion objective.
# v3 instead directly compares two decoded images (at s_i and at an extrapolated
# s > s_i) and penalizes any case where the extrapolated point is LESS visually
# different from the pre-disaster image than the anchor point -- i.e. it targets
# the exact observed failure mode (higher requested s producing an image that
# looks LESS damaged) with no classifier/probe in the loop at all.
import argparse
import csv
import random
...
# v3: floor used ONLY for the extrapolation target's slope denominator, not for
# s_i itself elsewhere. The linear-interpolation trend (z_post-z_pre)/s_i has
# slope 1/s_i; for the ~58% of pairs with near-zero real severity (s_i near the
# EPS_SI floor), extrapolating this trend out to s=1 with the raw 1/s_i slope
# blows up numerically (e.g. s_i=0.001 -> ratio up to 1000x), producing garbage
# out-of-distribution "extrapolation targets" that teach the model nothing
# meaningful -- discovered via v3 smoke test (monotonic-loss was trivially ~0 for
# low-s_i samples because the extrapolated image was already maximally, absurdly
# different from pre due to this blowup, not because the model had learned
# anything). This almost certainly also undermined v2's severity-probe
# extrapolation loss for the same low-severity majority of pairs.
EXTRAP_SLOPE_FLOOR = 0.15
...
def main():
    ...
    parser.add_argument("--lambda_monotonic_extrap", type=float, default=0.2,
                         help="Weight for the v3 pixel-space monotonicity ordering loss")
    parser.add_argument("--monotonic_margin_scale", type=float, default=0.1,
                         help="Required minimum increase in pre-image L1 distance per unit of (s_extrap - s_i)")
    parser.add_argument("--monotonic_extrap_start_step", type=int, default=3000,
                         help="Don't apply the monotonicity loss before this step (let eps loss stabilize first)")
    ...
    lambda_monotonic_extrap = float(args.lambda_monotonic_extrap)
    monotonic_margin_scale = float(args.monotonic_margin_scale)
    monotonic_extrap_start_step = int(args.monotonic_extrap_start_step)
    print(
        f"v3 monotonicity extrapolation loss: lambda={lambda_monotonic_extrap}, "
        f"margin_scale={monotonic_margin_scale}, start_step={monotonic_extrap_start_step}"
    )
    ...
    for step in pbar:
        ...
        need_endpoint_preds = (
            ...
            or (lambda_monotonic_extrap > 0.0 and step >= monotonic_extrap_start_step)
        )
        ...
        loss_monotonic_extrap = torch.tensor(0.0, device=device)
        extrap_mask = s_i < (1.0 - EPS_SI)
        if lambda_monotonic_extrap > 0.0 and step >= monotonic_extrap_start_step and extrap_mask.any():
            # v3: no frozen classifier/probe in the loop. Directly compare two
            # DECODED images -- the anchor at s=s_i (real pixel target exists)
            # and an extrapolated point at s_extrap in (s_i, 1] (no real pixel
            # target, same synthetic-overshoot construction as v2) -- and
            # penalize any case where the extrapolated point is LESS visually
            # different from the pre-disaster image than the anchor is. This
            # targets the exact failure mode observed empirically in v1/v2: mean
            # predicted severity rising through s=0.75 then DROPPING at s=1.
            u_extrap = torch.rand(batch_size, device=device)
            s_extrap = s_i + u_extrap * (1.0 - s_i)
            ratio_extrap = (s_extrap / s_i.clamp_min(EXTRAP_SLOPE_FLOOR)).view(batch_size, 1, 1, 1)
            z_target_extrap = z_pre + ratio_extrap * (z_post - z_pre)
            noise_extrap = torch.randn_like(z_pre)

            _, _, x0_pred_extrap = forward_eps_and_x0(
                unet=unet,
                scheduler=scheduler,
                t=t,
                z_target=z_target_extrap,
                z_pre=z_pre,
                s=s_extrap,
                noise=noise_extrap,
                text_emb=text_emb,
            ...
            img_extrap = decode_latents_with_grad(vae, x0_pred_extrap)
            if img_pred_s1 is not None:
                img_anchor = img_pred_s1
            else:
                img_anchor = decode_latents_with_grad(vae, x0_pred_s1)

            d_anchor = (img_anchor.float() - pre.float()).abs().mean(dim=[1, 2, 3])
            d_extrap = (img_extrap.float() - pre.float()).abs().mean(dim=[1, 2, 3])

            required_margin = monotonic_margin_scale * (s_extrap - s_i).clamp_min(0.0)
            per_sample_loss = F.relu(d_anchor + required_margin - d_extrap)
            mask_f = extrap_mask.float()
            loss_monotonic_extrap = (per_sample_loss * mask_f).sum() / mask_f.sum().clamp_min(1.0)
            loss = loss + lambda_monotonic_extrap * loss_monotonic_extrap
        ...
        pbar.set_description(
            ...
            f"mono={loss_monotonic_extrap.item():.4f} "
        ...
        if step % cfg.logging.print_every == 0:
            ...
            print(
                ...
                f"loss_monotonic_extrap={loss_monotonic_extrap.item():.6f} "
