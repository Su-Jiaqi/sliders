...
# v2 addition: severity-probe-guided extrapolation loss. See
# claudecode/paper_draft/calibrated_severity_v2_improvement_plan.md section 1.
IMAGENET_MEAN = torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1)
IMAGENET_STD = torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1)


class SeverityProbeHead(nn.Module):
    """
    Frozen MobileNetV3-Small (the same clean xBD classifier used throughout this
    paper's evaluation) + a frozen linear layer whose weights are the exported
    RidgeCV severity probe (see export_severity_probe.py). Fully differentiable:
    takes a batch of images already in [-1, 1] (the VAE decode's native range) and
    returns a continuous predicted severity (0-3 scale) per image, so it can be
    used as a training loss, not just a post-hoc evaluation metric.
    """

    def __init__(self, classifier_ckpt: str, probe_npz: str, device: torch.device, img_size: int = 224):
        super().__init__()
        self.img_size = img_size

        backbone = models.mobilenet_v3_small(weights=None)
        in_dim = backbone.classifier[-1].in_features
        backbone.classifier[-1] = nn.Linear(in_dim, 2)
        ...
        coef = torch.from_numpy(probe["coef"]).float()
        intercept = float(probe["intercept"])
        self.severity_head = nn.Linear(coef.shape[0], 1)
        with torch.no_grad():
            self.severity_head.weight.copy_(coef.view(1, -1))
            self.severity_head.bias.copy_(torch.tensor([intercept]))
        ...
        self.backbone.classifier[-1].register_forward_hook(_hook)

        self.to(device)
        self.register_buffer("mean", IMAGENET_MEAN.to(device), persistent=False)
        self.register_buffer("std", IMAGENET_STD.to(device), persistent=False)

    def forward(self, images_neg1_to_1: torch.Tensor) -> torch.Tensor:
        """images_neg1_to_1: (B, 3, H, W) in [-1, 1] (VAE decode's native range)."""
        x = (images_neg1_to_1 + 1.0) / 2.0  # -> [0, 1]
        resize_to = int(self.img_size * 1.15)
        x = F.interpolate(x, size=(resize_to, resize_to), mode="bilinear", align_corners=False)
        top = (resize_to - self.img_size) // 2
        x = x[:, :, top:top + self.img_size, top:top + self.img_size]
        x = (x - self.mean) / self.std
        self.backbone(x)
        feat = self._feat
        severity = self.severity_head(feat).squeeze(-1)
        return severity
...
def main():
    ...
    parser.add_argument("--severity_probe_npz", type=str, required=True,
                         help="Exported RidgeCV probe weights from export_severity_probe.py")
    parser.add_argument("--severity_probe_classifier_ckpt", type=str, required=True,
                         help="Clean classifier checkpoint whose features the probe was fit on")
    parser.add_argument("--lambda_severity_extrap", type=float, default=0.15,
                         help="Weight for the v2 severity-probe-guided extrapolation loss")
    parser.add_argument("--severity_extrap_start_step", type=int, default=3000,
                         help="Don't apply the extrapolation loss before this step (let eps loss stabilize first)")
    ...
    severity_probe_head = SeverityProbeHead(
        classifier_ckpt=args.severity_probe_classifier_ckpt,
        probe_npz=args.severity_probe_npz,
        device=device,
    )
    lambda_severity_extrap = float(args.lambda_severity_extrap)
    severity_extrap_start_step = int(args.severity_extrap_start_step)
    print(
        f"v2 severity-probe extrapolation loss: lambda={lambda_severity_extrap}, "
        f"start_step={severity_extrap_start_step}"
    )
    ...
    for step in pbar:
        ...
        loss_severity_extrap = torch.tensor(0.0, device=device)
        extrap_mask = s_i < (1.0 - EPS_SI)
        if lambda_severity_extrap > 0.0 and step >= severity_extrap_start_step and extrap_mask.any():
            # s > s_i has no real pixel target for this pair; reuse the same
            # linear-interpolation prior (Eq.1) already used everywhere else in
            # this method to construct a plausible denoising target, extended
            # past its normal [0, s_i] domain -- the actual supervision signal
            # is the severity-probe loss below, not pixel fidelity to this
            # synthetic overshoot target. See v2 improvement plan section 1.
            u_extrap = torch.rand(batch_size, device=device)
            s_extrap = s_i + u_extrap * (1.0 - s_i)
            ratio_extrap = (s_extrap / s_i.clamp_min(EPS_SI)).view(batch_size, 1, 1, 1)
            ...
            pred_severity = severity_probe_head(img_extrap)
            target_severity = s_extrap * 3.0

            per_sample_loss = F.mse_loss(pred_severity.float(), target_severity.float(), reduction="none")
            mask_f = extrap_mask.float()
            loss_severity_extrap = (per_sample_loss * mask_f).sum() / mask_f.sum().clamp_min(1.0)
            loss = loss + lambda_severity_extrap * loss_severity_extrap
        ...
        pbar.set_description(
            ...
            f"sevx={loss_severity_extrap.item():.4f} "
        ...
        if step % cfg.logging.print_every == 0:
            ...
            print(
                ...
                f"loss_severity_extrap={loss_severity_extrap.item():.6f} "
