...
from cyclegan_eval_metrics import (
    compute_cas,
    compute_clip_i,
    compute_dino_i,
...
def collect_palette_triplets(
    palette_result_dir: Path, data_root: Path, split: str
    ...
    if not palette_result_dir.exists():
        raise FileNotFoundError(f"Missing palette result dir: {palette_result_dir}")

    triplets: List[Tuple[str, Path, Path, Path]] = []
    for out_path in sorted(palette_result_dir.glob("Out_*.png")):
        stem = out_path.stem[len("Out_") :]
        gt_path = post_dir / f"{stem}.png"
        pre_path = pre_dir / f"{stem}.png"
        if gt_path.exists() and pre_path.exists():
            triplets.append((stem, out_path, gt_path, pre_path))
    return triplets


def evaluate_palette_split(
    split: str,
    result_dir: Path,
    data_root: Path,
    metric_size: int,
    ...
    triplets = collect_palette_triplets(result_dir, data_root, split)
    if not triplets:
        raise ValueError(f"No matched Out_* pairs for split={split} in {result_dir}")
...
def main() -> None:
    parser = argparse.ArgumentParser("Palette generation metric evaluator")
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/eval/palette")
    args = parser.parse_args()

    test_result_dir = resolve_path(args.test_result_dir)
    train_result_dir = resolve_path(args.train_result_dir)
    ...
    with open(output_dir / "palette_metrics.json", "w", encoding="utf-8") as f:
    ...
    with open(output_dir / "palette_metrics.csv", "w", newline="", encoding="utf-8") as f:
    ...
    print(f"\nSaved: {output_dir / 'palette_metrics.json'}")
    print(f"Saved: {output_dir / 'palette_metrics.csv'}")
