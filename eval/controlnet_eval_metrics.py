...
def pil_to_torch_rgb01(img: Image.Image, size: int) -> torch.Tensor:
    tf = transforms.Compose([transforms.Resize((size, size)), transforms.ToTensor()])
    return tf(img)


def matched_triplets(pred_dir: Path, gt_post_dir: Path, pre_dir: Path) -> List[Tuple[str, Path, Path, Path]]:
    pred_map = list_image_map(pred_dir)
    gt_map = list_image_map(gt_post_dir)
    pre_map = list_image_map(pre_dir)
    common = sorted(set(pred_map) & set(gt_map) & set(pre_map))
    return [(stem, pred_map[stem], gt_map[stem], pre_map[stem]) for stem in common]
...
def compute_clip_i(
    ...
    with torch.no_grad():
        for i in range(0, len(triplets), batch_size):
            ...
            cos = (pred_feat * gt_feat).sum(dim=-1)
...
def evaluate_split(
    ...
    if not pred_dir.exists():
        raise FileNotFoundError(f"Missing pred split dir: {pred_dir}")
    if not gt_post_dir.exists():
        raise FileNotFoundError(f"Missing gt post dir: {gt_post_dir}")
    if not pre_dir.exists():
        raise FileNotFoundError(f"Missing pre dir: {pre_dir}")

    triplets = matched_triplets(pred_dir, gt_post_dir, pre_dir)
    if not triplets:
        raise ValueError(f"No matched files for split={split}")
...
def main() -> None:
    parser = argparse.ArgumentParser("ControlNet generation metric evaluator")
    parser.add_argument("--pred_root", type=str, required=True, help="e.g. outputs/baselines/controlnet/socalfire")
    ...
    parser.add_argument("--metric_size", type=int, default=256, help="Size for LPIPS/SSIM/PSNR")
    parser.add_argument("--feat_size", type=int, default=224, help="Size for FID/CLIP/DINO features")
    ...
    parser.add_argument("--classifier_ckpt", type=str, default=None, help="Optional CAS classifier checkpoint")
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/eval/controlnet")
    ...
    with open(output_dir / "controlnet_metrics.json", "w", encoding="utf-8") as f:
    ...
    with open(output_dir / "controlnet_metrics.csv", "w", newline="", encoding="utf-8") as f:
    ...
    print(f"\nSaved: {output_dir / 'controlnet_metrics.json'}")
    print(f"Saved: {output_dir / 'controlnet_metrics.csv'}")
